...
#include "<TO BE DONE, TIMER COUNTER>"     /* layer 1 timer counter device driver          */
#include "<TO BE DONE, INTERRUPT CONTROLLER>" /* layer 1 interrupt controller device driver */
...
int main()
{
     ...
     */
    Status = XTmrCtr_Initialize(<TO BE DONE, TIMER INSTANCE POINTER>, 
                                <TO BE DONE, TIMER DEVICE ID>);
    if (Status != XST_SUCCESS)
    {
     ...
     */
    Status = XTmrCtr_SelfTest(<TO BE DONE, TIMER INSTANCE POINTER>, 
                              TIMER_COUNTER_0);
    if (Status != XST_SUCCESS)
    {
     ...
     */
    XTmrCtr_SetHandler(<TO BE DONE, TIMER INSTANCE POINTER>, 
                       <TO BE DONE, TIMER HANDLER>, &TimerCounter);
     ...
     */
    XTmrCtr_SetOptions(<TO BE DONE, TIMER INSTANCE POINTER>, TIMER_COUNTER_0,
                       XTC_INT_MODE_OPTION | <TO BE DONE, TIMER RELOAD OPTION>);

    /* Set a reset value for the timer counter such that it will expire
     * earlier than letting it roll over from 0, the reset value is loaded
     * into the timer counter when it is started
     */
    XTmrCtr_SetResetValue(<TO BE DONE, TIMER INSTANCE POINTER>, 
                           TIMER_COUNTER_0, RESET_VALUE);

    /* Start the timer counter such that it's incrementing 
     */
    XTmrCtr_Start(<TO BE DONE, TIMER INSTANCE POINTER>, TIMER_COUNTER_0);
    ...
    Status = XIntc_Connect(&InterruptController,
                           <TO BE DONE, TIMER INTERRUPT ID>,
                           (XInterruptHandler)XTmrCtr_InterruptHandler,
                           <TO BE DONE, TIMER INSTANCE POINTER>);
    if (Status != XST_SUCCESS)
    {
...
void TimerCounterHandler(void *CallBackRef, Xuint8 TimerCounterNumber)
{
     ...
     * by the time slice
     */
    Leds = <TO BE DONE, READ THE CURRENT VALUE OF THE GPIO> & LED;
    if (Leds)
    {
