...
//counts up to parameter no reset by itself
module el_counter(
		input clk,
		input reset,
		input in_ready,			//assert this for one cycle when providing module with fresh input
	...
	always @ (posedge clk)
	begin
		if (reset) //done???????????????????????????????????????? --> only want done on for 1 cycle!!!
			begin
				count <= 'b0;
			end
		else if (in_ready) //don't keep adding if done
			begin
				count <= count + 1'b1;
			end
	end
...
module matrixmultiplier (
	...
	floating_point_v6_1 fmul (
	  ...
	  .m_axis_result_tready((run_count == 11)/*runcount_tvalid*/), // input m_axis_result_tready
	...
	floating_point_add_sub_v6_1 fadd1 (
	  .aclk(clk), // input aclk
	  .s_axis_a_tvalid((run_count == 12)/*runcount_tvalid*/), // input s_axis_a_tvalid
	  .s_axis_a_tready(a1_tready), // output s_axis_a_tready
	  .s_axis_a_tdata(product_tdata), // input [31 : 0] s_axis_a_tdata
	  .s_axis_b_tvalid((run_count == 12) /*runcount_tvalid*/), // input s_axis_b_tvalid
	  .s_axis_b_tready(b1_tready), // output s_axis_b_tready
	  .s_axis_b_tdata((element_count == 0 || (sum_tvalid & (element_count ==4))) ? 32'b0 : sum_tdata ), // input [31 : 0] s_axis_b_tdata
	...
	el_counter #(.N(4)) element_counter( //almost worked before without the 5 param overide!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		.clk(clk),
		.reset(reset || ((element_count == 4) & sum_tvalid)),
		.in_ready((run_count == 12)/*sum_tvalid*/),
		//.done(element_ready),
		.count(element_count)
	);
